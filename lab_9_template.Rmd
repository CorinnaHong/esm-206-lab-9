...
# Anything else we can pick out re: trends, outliers, etc.? 
# What is the general trend? Any outliers? As sqft increase, price increases
# Is there reason enough for us to omit it?
...
```{r saturated, echo = FALSE}

# a. Saturated model: 

homes_lm1<- lm(Price ~ City + Bedrooms + Bathrooms + SqFt + PricePerSqFt + Status, data = homes_sub) # Price as a fucntion of..., where to find data

# This^ is a bad model. This is because price per sqft is calculated from the dependent variable Price, This can lead to biased overfitting of model results

# Using the coefficients, the equation for this model is : Price = -626690 - 76863(SLO) - 9068.2(Atas) + 120160(SM-O) ... 

# Does this model make sense? NO. It is telling me that as you add 1 bedroom, the price decreases.

# Lets simplify: # of bedrooms, # of bathrooms, and sqft all tell us the same thing (how big the house is) choose just 1 to include
...
# If everything else stays the same, A house in SLO will sell for $34525.4 more on average than Arroyo Grande (Always compared to reference varable)

# b. Model summary:

summary(homes_lm2)

# p-values of coefficients answer "does this variable significantly predict the model outcome?"
...
# Type class(homes_sub$City) in console
...
levels(homes_sub$Status) # Check the levels the first is now "regular"
...
sat_aic <- AIC(homes_lm1) # 10,699.25
final_aic <- AIC(homes_lm3) # 11,148

# Answer: which would you pick? 

# If you only look at AIC, you would end up picking homes_lm1. We should still pick homes_lm3 because heteroscedasticity is OK, residuals normality definitely looks good, and conceptually and mathematically my model is making sense 
